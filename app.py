...
def save_inquiry(data):
    """Store submitted contact inquiries locally in a JSON file."""
    inquiries = []
...
def contact_api():
    """Handles leadership inquiries and contact submissions with validation."""
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"Thank you, {name}! Your inquiry has been sent to Mukesh Gupta. You will receive a response shortly.",
            "mailto_fallback": f"mailto:mgsmukeshgupta@gmail.com?subject=Leadership%20Inquiry%20from%20{name}&body={message}"
        }), 200
